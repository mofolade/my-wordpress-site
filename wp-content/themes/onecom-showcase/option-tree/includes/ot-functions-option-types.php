        ...
        foreach( $field_value as $key => $value ) {
            ...
            if ( in_array( 'variants', $ot_recognized_google_fonts_fields ) ) {
                ...
                foreach ( ot_recognized_google_font_variants( $field_id, $family ) as $variant_key => $variant ) {
                    ...
                    /* By-default checking all the variants, because onecom CDN hosted fonts have the variants already included */
                    /* More details: WPIN-2558 */
                    echo '<input type="checkbox" name="' . esc_attr( $field_name ) . '[' . $key . '][variants][]" id="' . esc_attr( $field_id ) . '-' . $key . '-' . $variant . '" value="' . esc_attr( $variant ) . '" ' . checked(true , true, false)  . ' class="option-tree-ui-checkbox ' . esc_attr( $field_class ) . '" disabled />';
            ...
            if ( in_array( 'subsets', $ot_recognized_google_fonts_fields ) ) {
                ...
                foreach ( ot_recognized_google_font_subsets( $field_id, $family ) as $subset_key => $subset ) {
                    echo '<p class="checkbox-wrap">';
                    /* By-default checking all the subsets, because onecom CDN hosted fonts have the subsets already included */
                    /* More details: WPIN-2558 */
                    echo '<input type="checkbox" name="' . esc_attr( $field_name ) . '[' . $key . '][subsets][]" id="' . esc_attr( $field_id ) . '-' . $key . '-' . $subset . '" value="' . esc_attr( $subset ) . '" ' . checked( true, true, false )  . ' class="option-tree-ui-checkbox ' . esc_attr( $field_class ) . '" disabled />';
  ...
  function ot_type_list_item( $args = array() ) {
    ...
    echo '<div class="format-setting type-list-item ' . ( $has_desc ? 'has-desc' : 'no-desc' ) . '">';
      ...
      echo '<div class="format-setting-inner">';
        ...
        /* description */
        $list_desc = $sortable ? __( 'You can re-order with drag & drop, the order will update after saving.', 'option-tree' ).' '.__( 'Please update the page after adding a new information block in order to edit or create a new block.', 'option-tree' ) : '';
    ...
    echo '<div class="format-setting type-measurement ' . ( $has_desc ? 'has-desc' : 'no-desc' ) . '">';
      ...
      echo '<div class="format-setting-inner">';
        ...
        echo '<select name="' . esc_attr( $field_name ) . '[1]" id="' . esc_attr( $field_id ) . '-1" class="option-tree-ui-select ' . esc_attr( $field_class ) . '">';
          ...
          // Removed blank option to resolve the issue
  ...
  function ot_type_slider( $args = array() ) {
    ...
    echo '<div class="format-setting type-slider ' . ( $has_desc ? 'has-desc' : 'no-desc' ) . '">';
      ...
      echo '<div class="format-setting-inner">';
        ...
        /* description */
        echo '<div class="list-item-description">' . __( 'You can re-order with drag & drop, the order will update after saving.', 'option-tree' ) . ' '.__( 'Please update the page after adding a new information block in order to edit or create a new block.', 'option-tree' ).'</div>';
