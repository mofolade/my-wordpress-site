...
/* Function to get google font family name by family-slug */
function ot_google_font_family($family_slug){
	...
	if(isset($family_slug) && strlen($family_slug)){
		if(array_key_exists($family_slug, $native_fonts)){
			return $native_fonts[$family_slug];
		}
	}

	$ot_google_fonts = get_theme_mod( 'ot_google_fonts', array() );

	if( isset($family_slug) && isset( $ot_google_fonts[$family_slug] ) ){
		return $ot_google_fonts[$family_slug]['family'];
	}
...
if ( ! function_exists( 'ot_load_google_fonts_css' ) ) {
    ...
    function ot_load_google_fonts_css() {
        ...
        if ( ! empty( $ot_set_google_fonts ) ) {
            ...
            foreach( $ot_set_google_fonts as $id => $fonts ) {
                ...
                foreach( $fonts as $font ) {
                    ...
                    if ( isset( $ot_google_fonts[$font['family']]['id']) ) {

                        $families[] = $ot_google_fonts[$font['family']]['id'];

                    }elseif(isset( $ot_google_fonts[$font['family']]['family'])){

                        $families[] = str_replace(" ", "-", strtolower($ot_google_fonts[$font['family']]['family']));

                    }else{
                        continue;
                    }

                    // We are skipping to add variants because new CDN hosted fonts will keep all of them included
                    ...
                    if ( isset( $variants ) ) {
                        $families[] = str_replace( ' ', '+', $ot_google_fonts[$font['family']]['id'] ) . $variants;
                    }*/
        ...
        if ( ! empty( $families ) ) {
            ...
            //wp_enqueue_style( 'ot-google-fonts',  admin_url('admin-ajax.php').'?action=oc_manage_fonts&url=https://fonts.googleapis.com/css?family=' .urlencode(implode( '%7C', $families ))  . $append, false, null );
            ...
            foreach ($families as $family){
                wp_enqueue_style( 'ot-google-fonts-via-onecom-cdn'.'-'.$family,  $fonts_api_url.'/'.$family.'/'.$family.'.css', false, null );
            }
