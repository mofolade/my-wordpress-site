...
if ( ! function_exists( 'ot_register_theme_options_page' ) ) {
  ...
  function ot_register_theme_options_page() {
    ...
    if ( function_exists( 'ot_register_settings' ) && OT_USE_THEME_OPTIONS ) {
      ...
      add_filter('option_page_capability_' . ot_options_id(), 'ot_capability_fx', 999);

            function ot_capability_fx($caps) {
                return $caps;
            }
...
if ( ! function_exists( 'ot_admin_scripts' ) ) {
  ...
  function ot_admin_scripts() {
    ...
    if(isset($_REQUEST['page']) && $_REQUEST['page'] == 'octheme_settings'){
    ...
    $localized_array = array(
      ...
      'upload_text'           => apply_filters( 'ot_upload_text', esc_html__( 'Add Media', 'option-tree' ) ),
  ...
  function ot_insert_css_with_markers( $field_id = '', $insertion = '', $meta = false ) {
    ...
    if ( file_exists( $filepath ) ) {
      ...
      if ( ! is_writable( $filepath ) ) {
        add_settings_error( 'option-tree', 'dynamic_css', sprintf( esc_html__( 'Unable to write to file %s.' ), '<code>' . $filepath . '</code>' ), 'error' );
        return false;
      }
  ...
  function ot_sections_view( $name, $key, $section = array() ) {
    ...
    <div class="option-tree-setting is-section">
      ...
      <div class="button-section">
        <a href="javascript:void(0);" class="option-tree-setting-edit option-tree-ui-button button left-item" title="' . sprintf( 'edit') . '">
          <span class="icon ot-icon-pencil"></span>' . esc_html__( 'Edit', 'option-tree' ) . '
        </a>
        <a href="javascript:void(0);" class="option-tree-setting-remove option-tree-ui-button button button-secondary light right-item" title="' . sprintf( 'Delete' ) . '">
      ...
      <div class="option-tree-setting-body">
        <div class="format-settings">
          <div class="format-setting type-text">
            <div class="description">' . sprintf( '<strong>Section Title</strong>: Displayed as a menu item on the Theme Options page.' ) . '</div>
        ...
        <div class="format-settings">
          <div class="format-setting type-text">
            <div class="description">' . sprintf( '<strong>Section ID</strong>: A unique lower case alphanumeric string, underscores allowed.' ) . '</div>
            <div class="format-setting-inner">
      ...
      <div class="option-tree-setting-body">
        <div class="format-settings">
          <div class="format-setting type-text wide-desc">
            <div class="description">' . sprintf( '<strong>Label</strong>: Displayed as the label of a form element on the Theme Options page.') . '</div>
            <div class="format-setting-inner">
        ...
        <div class="format-settings">
          <div class="format-setting type-text wide-desc">
            <div class="description">' . sprintf( '<strong>ID</strong>: A unique lower case alphanumeric string, underscores allowed.' ) . '</div>
            <div class="format-setting-inner">
        ...
        <div class="format-settings">
          <div class="format-setting type-select wide-desc">
            <div class="description">' . sprintf( '<strong>Type</strong>: Choose one of the available option types from the dropdown.', 'option-tree' ) . '</div>
            <div class="format-setting-inner">
        ...
        <div class="format-settings">
          <div class="format-setting type-textarea wide-desc">
            <div class="description">' . sprintf( '<strong>Description</strong>: Enter a detailed description for the users to read on the Theme Options page, HTML is allowed. This is also where you enter content for both the Textblock & Textblock Titled option types.', 'option-tree' ) . '</div>
            <div class="format-setting-inner">
        ...
        <div class="format-settings">
          <div class="format-setting type-textblock wide-desc">
            <div class="description">' . sprintf( '<strong>Choices</strong>: This will only affect the following option types: Checkbox, Radio, Select & Select Image.' ) . '</div>
            <div class="format-setting-inner">
...
function ot_update_google_fonts_after_save( $options ) {
  ...
  **/
  if( get_option( 'onecom_fresh_install' ) && get_option( 'onecom_fresh_install' ) === true ) {
    update_option( 'onecom_fresh_install', false );
  }
...
/**
 * Helper function to tell theme first time saving options tree
 * 
**/
function ot_override_options_callback() {
  if( ! get_option( 'ot_override_options' ) ) {
    update_option( 'ot_override_options', true );
    header("Refresh:0");
  }
}
function is_ot_fresh_site() {
  if( get_option( 'ot_override_options' ) && get_option( 'ot_override_options' ) == true ) {
    return false;
  }
  return true;
}
add_action( 'ot_after_theme_options_save', 'ot_override_options_callback', 1 );
...
function ot_fetch_google_fonts( $normalize = true, $force_rebuild = false ) {
  ...
  if ( $force_rebuild || ! is_array( $ot_google_fonts ) || empty( $ot_google_fonts ) ) {
    ...
    $ot_google_fonts = array();
        ...
        // get environment specific CDN fonts API
        $fonts_api_url = apply_filters("ot_onecom_fonts_api", "https://fonts-static.cdn-one.com/fonts/google");

        // append endpoint
        $fonts_api_url .= "/fonts.json";

        $ot_google_fonts_response = wp_safe_remote_get( $fonts_api_url, array('headers' => array("User-Agent" => "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/102.0.0.0 Safari/537.36"), 'sslverify' => false, 'timeout' => 15 ) );
    ...
    if ( 200 == wp_remote_retrieve_response_code( $ot_google_fonts_response ) ) {
      ...
      if ( $response_body = wp_remote_retrieve_body( $ot_google_fonts_response ) ) {
        ...
        $ot_google_fonts_data = json_decode( trim( $response_body ), true );
                ...
                if ( is_array( $ot_google_fonts_data ) && is_countable($ot_google_fonts_data) && count($ot_google_fonts_data) > 0 ) {

                    $ot_google_fonts = $ot_google_fonts_data;

                    // Sort font-families alphabetically
                    $ot_google_fonts = apply_filters("ot_sort_font_families", $ot_google_fonts);
