...
if ( ! class_exists( 'OT_Loader' ) ) {
  ...
  class OT_Loader {
    ...
    private function load_languages() {
      ...
      if ( OT_PLUGIN_MODE ) {
        ...
        if ( apply_filters( 'ot_child_theme_mode', false ) == true ) {
          ...
          $path = ltrim( end( $path ), '/' );
          define( 'OT_LANG_DIR', trailingslashit( trailingslashit( get_stylesheet_directory() ) . $path ) . trailingslashit( 'languages' ) );
          
        } else {
          
          $path = @explode( get_template(), str_replace( '\\', '/', dirname( __FILE__ ) ) );
          $path = ltrim( end( $path ), '/' );
          define( 'OT_LANG_DIR', trailingslashit( trailingslashit( get_template_directory() ) . $path ) . trailingslashit( 'languages' ) );
      ...
      if ( false == OT_THEME_MODE && false == OT_CHILD_THEME_MODE ) {
        ...
        if ( true == OT_CHILD_THEME_MODE ) {
          ...
          define( 'OT_URL', trailingslashit( trailingslashit( get_stylesheet_directory_uri() ) . $path ) );
            ...
            $somesd = @explode( get_template(), str_replace( '\\', '/', dirname( __FILE__ ) ) );
          $path = ltrim( end( $somesd ), '/' );
    ...
    private function hooks() {
      ...
      /* CDN fonts URL based on environment */
      add_filter( 'ot_onecom_fonts_api', array($this, 'ot_fonts_api_url'));

      /* Sort font families array */
      add_filter( 'ot_sort_font_families', array($this, 'sort_font_families'));
    ...
    public function change_image_button( $translation, $text, $domain ) {
      ...
      if ( $pagenow == apply_filters( 'ot_theme_options_parent_slug', 'themes.php' ) && 'default' == $domain && 'Insert into post' == $text ) {
        ...
        return apply_filters( 'ot_upload_text', __( 'Add Media', 'option-tree' ) );
      ...
      /**
       * Helper function to alphabetically sort array of font-families by their name
       * @param $a
       * @param $b
       * @return int
       */
      public function ot_cmp($a, $b) {
          return strcmp($a['id'], $b['id']);
      }

      public function sort_font_families($ot_google_fonts)
      {
          usort($ot_google_fonts, array($this, "ot_cmp"));
          return $ot_google_fonts;
      }

      /**
       * Helper function to return environment specific fonts API url
       * @return string
       */
      public function ot_fonts_api_url($url) {
          if(defined('MIDDLEWARE_URL')) {
              if(
                  strpos(MIDDLEWARE_URL, 'wpapi-test') !== false
                  || strpos(MIDDLEWARE_URL, 'wpapi-staging') !== false
                  || strpos(MIDDLEWARE_URL, 'wpapi-next') !== false
              ){
                  return 'https://fonts-static.cdn-one.com/fonts/google';
              }
          }
          return $url;
      }
