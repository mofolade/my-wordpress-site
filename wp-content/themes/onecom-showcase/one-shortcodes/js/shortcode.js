	...
	$( document ).ready( function() {
		...
		*/
		try {
			...
			if ($(".shinybox").length !== 0){
				var url = window.location.href;
				var cleanUrl;
				var urlCleaned = url.replace("https://", "");
				urlCleaned = urlCleaned.replace("http://", "");
				if (urlCleaned.indexOf("/") !== -1){
					cleanUrl = urlCleaned.split("/")[0];
				}else{
					cleanUrl = urlCleaned;
				}
				$.each($(".shinybox"), function(index, value){
					if (value.href.indexOf(cleanUrl) === -1){
						$(value).removeClass("shinybox");
