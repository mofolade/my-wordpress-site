	...
	class Onecom_Usage_Stats {
		...
		public function __construct() {
			if ( ! get_site_option( self::OC_CU_CONSENT ) ) {
				...
				// Hook to monitor plugin activations
			}
			...
			if ( isset( $_POST['oc-save-consent'] ) ) {
				if ( isset( $_POST[ self::OC_CU_CONSENT ] ) && $_POST[ self::OC_CU_CONSENT ] == 'on' ) {
					...
					if ( strpos( 'onecom-vcache', __DIR__ ) ) {
						$referrer = basename( dirname( dirname( dirname( __DIR__ ) ) ) );
					} else {
						$referrer = basename( dirname( dirname( __DIR__ ) ) );
					}
		...
		function prepare_payload() {
			...
			$package_features   = isset( $get_features['data'] ) ? $get_features['data'] : array();
		 ...
		 */
		function oc_toggle() {
			?><script type="text/javascript">function ocToggle() {jQuery('.oc-wht-data').toggle();}</script>
			<?php
		}
		...
		public function monitor_plugin_activation( $plugin, $network_activation ) {
			global $pagenow;
			if ( isset( $_GET['referrer'] ) && 'recommended' === $_GET['referrer'] ) {
			...
			if ( in_array( $plugin, $partner_plugins, true ) && ! $network_activation ) {
				// Plugin has been activated and is not a network activation
		...
		public function monitor_plugin_deactivations( $plugin ) {
			...
			if ( in_array( $plugin, $partner_plugins ) ) {
				$plugin_name = $this->get_plugin_name_from_slug( $plugin );
