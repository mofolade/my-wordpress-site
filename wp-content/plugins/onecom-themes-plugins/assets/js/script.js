(function($){
	$(document).ready(function(){
		...
		$(document).on( 'click', '.one-install, .one-installed', function(){
			...
			oc_validate_action(type).then(function(response){
				var isPremium, featureAction, actionType; //actionType to be sent only for success log.
				if (type === 'ptheme') {
					isPremium = 'true'
				}
				if (type === 'stheme') {
				...
				if (that.data('name') === 'onecom_install_theme') {
					featureAction = 'install';
					actionType    = 'wppremium_install_theme';
				}
				if (that.data('name') === 'onecom_activate_theme') {
					featureAction = 'activate'
					actionType    = 'wppremium_activate_theme';
				}
	...
	$(document).on("click", ".preview_link", function(){
		...
		var referrer = location.search;
		oc_add_preview_log(demo_id, 'preview',referrer);
	});
