...
if ( ! function_exists('onecom_discouraged_plugins_notice') ) {
	function onecom_discouraged_plugins_notice(){
		...
		if ( ! empty($crtical_active) && is_array($crtical_active) ) {
			...
			$text =  __('You are using one or more plugins from the list of plugins we discourage. We recommend that you deactivate them, to ensure the best performance of your website. ', OC_PLUGIN_DOMAIN);

			$link  = admin_url('admin.php?page=onecom-wp-discouraged-plugins');
			$title = __('View discouraged plugins', OC_PLUGIN_DOMAIN);
