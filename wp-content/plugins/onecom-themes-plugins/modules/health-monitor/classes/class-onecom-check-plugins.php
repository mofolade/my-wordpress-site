...
class OnecomCheckPlugins extends OnecomHealthMonitor {
	...
	public function check_inactive_plugins() {
		...
		foreach ( $all_plugins as $plugin_path => $plugin_data ) {
			...
			if ( ! empty($inactive_plugins) ) {
				...
				$title                  = __('Remove inactive plugins', $this->text_domain);
				$desc                   = sprintf(
					__('Your site currently has %d inactive plugins. Inactive plugins are tempting targets for hackers. We recommend removing any plugins that you no longer need.', $this->text_domain),
	...
	public function check_inactive_themes() {
		...
		$ignore_authors = ['one.com','the WordPress team'];
		...
		if ( ! empty($inactive_themes) ) {
			$title          = __('Remove inactive themes', $this->text_domain);
