...
trait OnecomHMTexts {
	...
	public function init_texts() {
		$this->text['uploads_index']       = [
			$this->action_title     => __('Reduce the amount of files in the uploads folder', $this->text_domain),
...
<p>#one.com block xmlrpc END</p></code>',
		...
		$this->text['inactive_plugins']    = [
			$this->action_title    => __('Remove inactive plugins', $this->text_domain),
			$this->overview        => __('Inactive plugins are tempting targets for hackers . We recommend removing any plugins that you no longer need .', $this->text_domain),
			$this->fix_button_text => __('Manage inactive plugins', $this->text_domain),
			$this->how_to_fix      => __('Select " Manage inactive plugins " and delete all plugins that you don\'t need .', $this->text_domain),
			$this->how_to_fix_lite => __('Go to "Plugins" in the left-side menu and delete plugins that you don\'t need. You can see all unused plugins in the "Inactive" tab.', $this->text_domain),


			$this->status_desc     => [
				$this->status_resolved => __('Your site has no inactive plugins. ', $this->text_domain),
				$this->status_open     => __('Your site has %d inactive plugins', $this->text_domain)
			]
		];

		$this->text['inactive_themes']      = [
			$this->action_title    => __('Remove inactive themes', $this->text_domain),
			$this->overview        => __('To enhance your site\'s security , we recommend removing any inactive themes except the default WordPress themes ( called " Twenty Twenty - Three " and similar ) and the theme you\'re currently using .', $this->text_domain),
			$this->fix_button_text => __('Manage your themes', $this->text_domain),
			$instruction_1 = __('Click the button " Manage your themes " to get to your themes overview . ', $this->text_domain),
			$instruction_2 = __("Hover over the theme you want to delete and select “Theme Details”.", $this->text_domain),
			$instruction_3 = __("Click “Delete” in the bottom-right corner.", $this->text_domain),
			$instruction_4 = __("Go to “Appearance” > “Themes” in the left-side menu.", $this->text_domain),
			...
			$this->status_desc     => [
				$this->status_resolved => __('Your site has no inactive themes .', $this->text_domain),
				$this->status_open     => __('Your site has %d inactive themes', $this->text_domain)
			]
		];
		...
		$this->text['core_updates']             = [
			...
			$this->fix_button_text => '',
			$this->how_to_fix      => sprintf(__( "Enable automatic minor WordPress core updates again, either by changing a setting in the plugin you use to manage updates or by changing a setting in wp-config. If you would like to know why updates are so important, check this guide: %sWhy you should always update WordPress%s.", $this->text_domain ),
		...
		$this->text['dis_plugin']           = [
			...
			$this->status_desc      => [
				$this->status_resolved => __('You are doing great! None of your installed plugins, are on our list of discouraged plugins.', $this->text_domain),
		...
		$this->text['woocommerce_sessions'] = [
			...
			$this->how_to_fix_lite  => __('Clear expired woocommerce session data', $this->text_domain),
			$this->upsell_text      => __('one.com Managed WordPress comes with an easy fix and more.', $this->text_domain) . '<a  class="onecom__open-modal"> ' . __('Free upgrade', $this->text_domain) . '</a>',
		...
		$this->text['debug_enabled']            = [
			$this->action_title     => __('Your site is set to log errors to a potentially public file', $this->text_domain),
			$this->overview         => __("Debug mode is generally used to gather more details about an error or site failure, but the file where the information is collected might be publicly available. This means that any information about your errors could be visible to all users. To enhance the security of your site, we recommend disabling debug mode as soon as you don’t need it anymore.", $this->text_domain),
			$instruction_1 = __('Go to your File Manager and open the file called " wp - config.php "', $this->text_domain),
			$instruction_2 = __("Scroll down to the line that says: <strong>define( \'WP_DEBUG_LOG\', true );</strong> in that file.", $this->text_domain),
			$instruction_3 = __("Change it to: <strong>define( \'WP_DEBUG_LOG\', false );</strong> and then click “Save” at the top of the page.", $this->text_domain),
			$this->how_to_fix       => sprintf(
				'<ol><li>%s</li><li>%s</li><li>%s</li></ol>',
				$instruction_1,
				$instruction_2,
			...
			$this->fix_confirmation => '',
			$this->status_desc      => [
				$this->status_resolved => __('Debug mode is disabled.', $this->text_domain),
				$this->status_open     => __('Debug mode is enabled for this site , potentially making error details publicly available .', $this->text_domain)
			]
		];
		$this->text['debug_log_size']           = [
			$this->action_title     => __('Your debug.log file is taking up too much space', $this->text_domain),
			$this->overview         => __("The debug.log file can take up a lot of your disk space . We recommend deleting it if you no longer need it .", $this->text_domain),
			$this->fix_button_text  => __('Delete file', $this->text_domain),
			$this->upsell_text      => __('one.com Managed WordPress comes with a quick fix so you can spend more time on your website, less on security', $this->text_domain) . '<a class="onecom__open-modal"> ' . __('Free upgrade', $this->text_domain) . '</a>',
			$this->how_to_fix       => __('Click " Delete file " below to remove the debug.log file automatically . To remove it manually , locate and delete the file using your File Manager or an ( S ) FTP client . In most cases , the debug.log file is found in the folder called " wp - content " .', $this->text_domain),
			$this->how_to_fix_lite  => __('Click " Delete file " below to remove the debug.log file automatically . To remove it manually , locate and delete the file using your File Manager or an ( S ) FTP client . In most cases , the debug.log file is found in the folder called " wp - content " .', $this->text_domain),
			$this->fix_confirmation => 'Debug.log file deleted',
			$this->status_desc      => [
				$this->status_resolved => __('Your debug.log file was deleted and is no longer taking up any disk space.', $this->text_domain),
				$this->status_open     => __('Your debug.log file takes up over 100 MB in disk space .', $this->text_domain)
			]
		];
