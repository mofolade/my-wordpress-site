...
if ( ! function_exists('oc_cb_config_page') ) {
	function oc_cb_config_page() {
		add_submenu_page(
			...
			'<span id="onecom_cookie_banner">'.__('Cookie Banner', OC_TEXTDOMAIN).'</span>',
			'manage_options',
...
if ( ! function_exists('oc_cb_scripts_admin') ) {
	function oc_cb_scripts_admin($hook_suffix) {
		...
		if ( $hook_suffix === '_page_onecom-wp-cookie-banner'
			...
			wp_enqueue_script('oc_cb_js', ONECOM_WP_URL . OCCB_ASSETS. '/' . $folder . 'js/cookie-banner-admin' . $extenstion . '.js', ['jquery'], ONECOM_WP_VERSION, true);
...
if ( ! function_exists('oc_cb_scripts_frontend') ) {
	function oc_cb_scripts_frontend() {
		...
		if ( $cookie_Enable ) {
			...
			wp_enqueue_script('oc_cb_js_fr', ONECOM_WP_URL . OCCB_ASSETS. '/' . $folder . 'js/cookie-banner-frontend' . $extenstion . '.js', ['jquery'], ONECOM_WP_VERSION, true);
...
if ( ! function_exists('oc_cb_settings') ) {
	function oc_cb_settings(){
		...
		wp_remote_request($blog_url, ['method' => 'PURGE']);
...
function oc_cb_cookie_consent(){
	...
	die(json_encode(array('error' => null, 'message' => $cookie_consent)));
}
...
if ( ! function_exists('oc_cookie_banner_notice') ) {
	function oc_cookie_banner_notice(){
		...
		$text =  __('Since you are using One.com Analytics plugin, we recommend that you enable a cookie banner. This banner tells your visitors about your site storing data about them.', OC_PLUGIN_DOMAIN);
		...
		echo sprintf(
			...
			$text, menu_page_url('onecom-wp-cookie-banner', false), __('Setup cookie banner', OC_PLUGIN_DOMAIN),
			admin_url('admin-post.php')."?action=oc_cb_notice&data=dismiss", __('Skip setup', OC_PLUGIN_DOMAIN)
		);
