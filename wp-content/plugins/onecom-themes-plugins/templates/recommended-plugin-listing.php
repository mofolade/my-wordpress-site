<div class="wrap" id="onecom-ui">
	...
	<?php
		// Get plugins data
		$recommended_plugins = onecom_fetch_plugins($recommended = true);
		$plugin_count        = onecom_plugins_count();
		if ( is_wp_error($recommended_plugins) ) {
			load_template(dirname(__FILE__) . '/wpapi-error.php');
		} else { ?>
						...
						<div class="one-plugin-card <?php echo ( count($recommended_plugins)  == 1 ) ? 'single-plugin' : ''; ?> <?php echo ( $plugin_installed ) ? 'installed' : ''; ?>">
							<div class="plugin-card-top">
								<div class="name column-name oc-flex">
									...
									if ( $icon_url != '' ) :
										...
										<span class="plugin-icon-wrapper icon-available">
											...
											<img src="<?php echo $icon_url;?>" alt="<?php echo $plugin->name; ?> " onerror="this.style.display='none'" />
										</span>
									<?php
									else :
								...
								<div class="desc column-description">
									<p><?php _e($plugin->short_description, OC_PLUGIN_DOMAIN); ?>
									<?php
