...
class Directories extends JobExecutable {
	...
	public function open($file, $mode) {
		...
		if ( false === $file_handle ) {
			$this->returnException(sprintf(__('Unable to open %s with mode %s', OC_PLUGIN_DOMAIN), $file, $mode));
		}
	...
	public function write($handle, $content) {
		...
		if ( false === $write_result ) {
			if ( $meta = \stream_get_meta_data($handle) ) {
				throw new \Exception(sprintf(__('Unable to write to: %s', OC_PLUGIN_DOMAIN), $meta['uri']));
		}
		} elseif ( strlen($content) !== $write_result ) {
			throw new \Exception(__('Out of disk space.', OC_PLUGIN_DOMAIN));
		}
