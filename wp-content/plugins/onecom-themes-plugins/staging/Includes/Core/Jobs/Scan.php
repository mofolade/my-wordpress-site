...
class Scan extends Job {
	...
	protected function directories() {
		...
		foreach ( $directories as $directory ) {
			// Not a valid directory or the directory is not readable then skip //
			if ( false === ( $path = $this->getPath($directory) ) || ! ( $directory->isReadable() ) ) {
				continue;
			}
	...
	protected function getPath($directory) {
        ...
        */
		...
		$path = str_replace(ABSPATH, '', $directory->getRealPath()); // replaced null with '' since supporting null is deprecated in php 8.1
