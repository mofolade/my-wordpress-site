...
class Data extends JobExecutable {
	...
	protected function step1() {
		...
		( class_exists('OCPushStats') ? \OCPushStats::generic_log('staging_error', "Search & Replace failed to update siteurl and homeurl in {$this->prefix}options {$this->db->last_error}", 'staging_error') : '' );
	...
	protected function step2() {
		...
		( class_exists('OCPushStats') ? \OCPushStats::generic_log('staging_error', "Search & Replace: Failed to update onecom_is_staging_site in {$this->prefix}options {$this->db->last_error}", 'staging_error') : '' );
	...
	protected function step3() {
		...
		( class_exists('OCPushStats') ? \OCPushStats::generic_log('staging_error', "Search & Replace: Failed to update/delete onecom_staging_existing_staging / onecom_staging_existing_live in {$this->prefix}options {$this->db->last_error}", 'staging_error') : '' );
	...
	protected function step4() {
		...
		( class_exists('OCPushStats') ? \OCPushStats::generic_log('staging_warning', "Failed to update rewrite_rules in {$this->prefix}options {$this->db->last_error}", 'staging_error') : '' );
	...
	protected function step5() {
		...
		( class_exists('OCPushStats') ? \OCPushStats::generic_log('staging_warning', "Failed to Reset permalink_structure in {$this->prefix}options {$this->db->last_error}", 'staging_error') : '' );
	...
	protected function step6() {
		...
		$resultOptions = $this->db->query(
			$this->db->prepare(
				"UPDATE {$this->prefix}options SET option_name = replace(option_name, %s, %s) WHERE option_name=%s", $this->db->prefix, $this->prefix, $this->db->prefix . "user_roles"
			)
		);
		...
		if ( $resultUserMeta === false ) {
			...
			( class_exists('OCPushStats') ? \OCPushStats::generic_log('staging_error', "Search & Replace: Failed to update options, option_name database table prefixes; {$this->db->last_error}", 'staging_error') : '' );
