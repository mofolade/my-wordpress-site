...
class Delete extends Job {
	...
	private function getCloneRecords($name = null) {
		...
		$this->clone->stagingSize = $this->getDirectorySizeInclSubdirs($this->clone->path);
		return $this->clone;
	}
	...
	public function deleteFinish() {
		...
		if ( false === update_option("onecom_staging_existing_staging", $existingClones, 'no') ) {
		...
		$stagingSize = $this->clone->stagingSize;
		(class_exists('OCPushStats') ? \OCPushStats::push_stats_event_staging('delete', 'staging', "$totalTimeMS", "$stagingSize") : '');
