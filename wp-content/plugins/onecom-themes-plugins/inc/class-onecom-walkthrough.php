	...
	class Onecom_Walkthrough {
		...
		function walkthrough_footer_scripts() {
			...
			if ( $show_pointer && $id == '#onecom_plugins' ) {
				...
				$this->make_pointer_script($id, $options, __('End tour', OC_PLUGIN_DOMAIN), $button2, $function, $parent, $screen);
		...
		public function onecom_health_monitor_tour() {
			$health_mn = array(
				...
				self::CONTENT => '<div class="img-holder"><img class="oc-walk-img" src=' . ONECOM_WP_URL . 'assets/images/hs_walk.svg /></div>'
									...
									. '<p>' . sprintf(__('%1$s and %2$s keeps an eye on any issues, spammers, vulnerabilities and lets you fix most of them with just 1-click.', OC_PLUGIN_DOMAIN), '<b>Health Monitor</b>', '<b>Spam Protection</b>') . '</p><p>'.__('The tour can be restarted anytime from the dashboard.', OC_PLUGIN_DOMAIN).'</p>',
		...
		public function onecom_performance_cache_tour() {
			...
			$performance_cache = array(
				...
				self::CONTENT => '<div class="img-holder"><img class="oc-walk-img" src=' . ONECOM_WP_URL . 'assets/images/vcache_walk.svg /></div>'
								...
								. '<p>' . sprintf(__('%1$s, %2$s and %3$s will enhance the speed of your website and improve its overall performance.', OC_PLUGIN_DOMAIN), '<b>Performance Cache</b>', '<b>CDN</b>', '<b>WP Rocket</b>'). '</p>',
				self::PARENT  => true
			);
		...
		public function onecom_utility_tour() {
			...
			$utility = array(
				...
				self::CONTENT => '<div class="img-holder"><img class="oc-walk-img" src=' . ONECOM_WP_URL . 'assets/images/utility_walk.svg /></div>'
								. '<h4>Utility Tools</h4>'
								. '<p>' . sprintf(__('Add features such as an %1$s, %2$s or a %3$s to increase the usability of your site.', OC_PLUGIN_DOMAIN), '<b>Error Page</b>', '<b>Cookie Banner</b>', '<b>Maintenance Mode</b>'). '</p>',
				self::PARENT  => true
			);
