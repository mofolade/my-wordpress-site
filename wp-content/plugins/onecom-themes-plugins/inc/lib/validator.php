...
if (  ! function_exists('oc_validator_load_textdomain') ) {
	function oc_validator_load_textdomain() {
		...
		if ( $current_locale === 'fi' ) {
			load_textdomain(OC_VALIDATOR_DOMAIN, dirname(__FILE__) . '/languages/validator-fi_FI.mo');
		} else if ( $current_locale === 'nb_NO' ) {
			load_textdomain(OC_VALIDATOR_DOMAIN, dirname(__FILE__) . '/languages/validator-no_NO.mo');
			...
			load_textdomain(OC_VALIDATOR_DOMAIN, dirname(__FILE__) . '/languages/validator-en_GB.mo');
		}
	}
}
...
if (  ! function_exists('onecom_upgrade_modal') ) {
	function onecom_upgrade_modal() {
		...
		if ( isset($_POST['action']) && $_POST['action'] === 'show_plugin_dependent_popup' ) {
			...
			wp_send_json(array('success' => true,"data" => $dependent_plugin));
		} else { ?>
...
add_filter('onecom_premium_inline_badge', $oc_inline_badge_fn, 10, 3); // attach with MM
...
function oc_set_redesign_notice_forever_callback(){
	...
	wp_send_json(array( "status" => $status,"message" => $message));wp_die();
}
			...
			<div class="oc-plugin-redesign-notices">
				<div class="oc-plugin-redesign-notices-wrap">
					<div class="oc-plugin-redesign-notices-desc">
						...
						<a class="oc-plugin-redesign-cta" href="<?php menu_page_url("onecom-wp-health-monitor", true);?>"><?php echo __('Explore plugins', OC_VALIDATOR_DOMAIN)?></a>
					</div>
					<div class="oc-plugin-redesign-notices-img">
						<img src="<?php echo $plugin_redesign_img;?>" alt="<?php echo __('one.com', OC_VALIDATOR_DOMAIN);?>">
					</div>
				</div>
			</div>
		</div>
