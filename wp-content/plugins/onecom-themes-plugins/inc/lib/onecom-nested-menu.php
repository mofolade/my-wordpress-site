	...
	final class Onecom_Nested_Menu {
		...
		public function onecom_register_menu() {
			...
			if ( $this->onecom_plugin_activated() && version_compare($generic_plugin_ver, '4.0.0', '>=') ) {
				add_submenu_page( self::ONECOM_MENU_SLUG,  self::WP_ROCKET_TITLE,  self::WP_ROCKET_TITLE, 'manage_options', 'onecom-wp-rocket', array(
					'Onecom_Wp_Rocket',
					'wp_rocket_page'
				), 2 );
			} else if ( is_plugin_active(self::ONECOM_VCACHE_FILE) &&
			...
			if ( is_plugin_active(self::ONECOM_VCACHE_FILE) &&
				version_compare($this->oc_get_plugin_version(self::ONECOM_VCACHE_FILE), '2.0', '>=') ) {
				add_submenu_page( self::ONECOM_MENU_SLUG, self::WP_PCACHE_TITLE,  self::WP_PCACHE_TITLE, 'manage_options', 'onecom-vcache-plugin', array(
					'OCVCaching',
					'cache_settings_page'
				), 2 );
			...
			} elseif ( $this->onecom_plugin_activated()
						&& is_multisite() || ( get_option('siteurl') !== get_option('home') ) ) {
				add_submenu_page(self::ONECOM_MENU_SLUG, 'Staging', '<span id="onecom_staging">Staging</span>', 'manage_options', 'onecom-wp-staging-blocked', '', 7);
			}
