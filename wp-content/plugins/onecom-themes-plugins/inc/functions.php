...
if ( ! function_exists('onecom_plugins_count') ) {
	function onecom_plugins_count() {
		...
		$r_plugins = onecom_fetch_plugins($recommended = true);
		if ( is_array($r_plugins) ) {
			$plugins['recommended'] = count($r_plugins);
		} else {
			$plugins['recommended'] = '';
		}
		...
		$all_d_plugins       = onecom_fetch_plugins($recommended = false, $discouraged = true);
...
if ( ! function_exists('onecom_install_theme_callback') ) {
	function onecom_install_theme_callback() {
		...
		if (
			...
			$response[ 'type' ]    = 'error';
			$response[ 'message' ] = __('WordPress is being upgraded. Please try again later.', OC_PLUGIN_DOMAIN);
		...
		$title = sprintf(__('Installing theme', OC_PLUGIN_DOMAIN));
		$nonce = 'theme-install';
		$url   = add_query_arg(
			array(
		...
		$default_error_message = __('Something went wrong. Please contact the support at One.com.', OC_PLUGIN_DOMAIN);
		...
		if ( is_wp_error($result) ) {
			...
			$status['errorMessage'] = __('Unable to connect to the file system. Please contact the support at One.com.', OC_PLUGIN_DOMAIN);
		...
		if ( $result == true ) {
			...
			$response[ 'message' ] = __('Theme installed successfully', OC_PLUGIN_DOMAIN);

			if ( $redirect != false ) {
				$button_html = '<span class="action-text one-activate-theme">'.__('Activate', OC_PLUGIN_DOMAIN).'</span>';
			} else {
				if ( $network ) {
				...
				$button_html = '<a href="'.$activate_url.'">'.__('Activate', OC_PLUGIN_DOMAIN).'</a>';
			}
		...
		if ( ( ! $plugins ) || empty($plugins) ) {
			...
			if ( is_wp_error($response) ) {
				if ( isset($response->errors[ 'http_request_failed' ]) ) {
					$errorMessage = __('Connection timed out', OC_PLUGIN_DOMAIN);
				} else {
				...
				if ( wp_remote_retrieve_response_code($response) != 200 ) {
					...
					if ( ! empty($body) && $body->success ) {
						...
						if ( $body->error == 'RESOURCE NOT FOUND' ) {
							...
							$errorMessage  = __('Sorry, no compatible plugins found with your version of WordPress and PHP.', OC_PLUGIN_DOMAIN).'&nbsp;<a href="'.$try_again_url.'">'.__('Try again', OC_PLUGIN_DOMAIN).'</a>';
						} else {
							echo $body->error;
						}
					}
...
if ( ! function_exists('onecom_install_plugin_callback') ) {
	function onecom_install_plugin_callback($isAjax = true, $pluginSlugParam = '') {
		...
		$title = sprintf(__('Installing plugin', OC_PLUGIN_DOMAIN));
		...
		if ( $result == true ) {
			...
			$response[ 'type' ]    = 'success';
			$response[ 'message' ] = __('Plugin installed successfully', OC_PLUGIN_DOMAIN);
			...
			if ( $redirect == false || $redirect == '' || is_multisite() ) {
				$button_html = '<a href="'.$activateUrl.'" class="activate-plugin btn button_1">'.__('Activate', OC_PLUGIN_DOMAIN).'</a>';
			} else {
				$button_html = '<a class="activate-plugin activate-plugin-ajax btn button_1" href="javascript:void(0)" data-action="onecom_activate_plugin" data-redirect="'.$redirect.'" data-slug="'.$status['file'].'" data-name="'.$plugin_name.'">'.__('Activate', OC_PLUGIN_DOMAIN).'</a>';
			}
			$response[ 'button_html' ] = $button_html;
